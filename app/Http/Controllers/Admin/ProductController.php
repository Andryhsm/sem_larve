...
class ProductController extends Controller
{
	...
	public function index()
	{
		...
		Session::forget('brand_array');
		$product_managers = $this->admin_repository->getByRole(2);
		$product_status = $this->product_status_repository->getAll();
		...
		foreach ($products as $key => $product) {
			if($product->brand_id != '')
			{
				if(isset( $product->brand->parent['brand_name']) && isset($product->brand['brand_name']))
		            $all_brands[$product->brand_id] = $product->brand->parent['brand_name']." ".$product->brand['brand_name'];
		        else{
		            if(isset($product->brand['brand_name']))
		                $all_brands[$product->brand_id] = $product->brand['brand_name'];
		            }
			}
			//$all_brands[$product->brand_id] = $product->brand->brand_name; 
		}
		...
		asort($brands);
		//$brands = $this->brand_repository->getAllBrand();
		return view('admin.product.list', compact('product_managers', 'product_status', 'brands'));
	}
	...
	public function index_product()
	{
		...
		if(auth()->guard('admin')->user()->role_id == 2){
		   $products = $this->product_repository->getByProductManager($user_id);
		}else{
			$products = $this->product_repository->getAll();
		}
	...
	{	
		...
		if(Input::has('brand_id') || Input::has('product_manager') || Input::has('status_id') || Input::has('before_at') || Input::has('after_at') || Input::has('show_doublon')){

			if(Input::has('show_doublon')){
				$product_ids = null;
				$array_product_ids = [];
				
				if(auth()->guard('admin')->user()->role_id == 2){
					//Show doublon product manager user
					$product_ids = $this->product_repository->getDoublesByManager($admin_id);
					foreach ($product_ids as $product_id) {
						$array_product_ids[] = $product_id->product_id;	
					}
				}else{
					//Show doublon admin user
					$product_ids = $this->product_repository->getDoubles();
					foreach ($product_ids as $product_id) {
						$array_product_ids[] = $product_id->target_id;	
					}	
				}
				$products = $this->product_repository->getProductsByArrayId($array_product_ids);
			} else {
				if(auth()->guard('admin')->user()->role_id == 2){
					$products = $this->product_repository->getFilterProductByManager($request->all(), $admin_id);
				}else{
					$products = $this->product_repository->getFilterProduct($request->all());
				}
				if(Input::has('brand_id'))
					Session::put('brand_id', Input::get('brand_id'));
				if(Input::has('product_manager'))
					Session::put('product_manager', Input::get('product_manager'));
				if(Input::has('status_id'))
					Session::put('status_id', Input::get('status_id'));
				if(Input::has('before_at'))
					Session::put('before_at', Input::get('before_at'));
				if(Input::has('after_at'))
					Session::put('after_at', Input::get('after_at'));

			}	
			$data_tables = Datatables::collection($products);
		}else{
			if(auth()->guard('admin')->user()->role_id == 2){
				$products = $this->product_repository->getByProductManager($admin_id);
				$data_tables = Datatables::collection($products);
			}else{
				$data_tables = Datatables::collection(collect([]));
			}
		}
		...
		$data_tables->EditColumn('product_name', function ($product) {
			...
			$brand_array = (Session::has('brand_array')) ? Session::get('brand_array') : [];
	     	
	     	if(count($product->brand['parent']) > 0){
	     		if(isset( $product->brand->parent['brand_name']) && isset($product->brand['brand_name'])){
	     			$brand_array[$product->brand->parent['brand_id']] = $product->brand->parent['brand_name']." ".$product->brand['brand_name'];
	     	    	Session::put('brand_array', $brand_array);
					...
					if(isset($product->brand['brand_name'])){
						...
						$brand_array[$product->brand['brand_id']] = $product->brand['brand_name'];
			    		Session::put('brand_array', $brand_array);
	...
	public function create()
	{
		...
		return view('admin.product.form', compact('categories', 'product','images','attribute_sets','role_admins', 'product_is_active','brands','tags'));
	}
...
//		return redirect()->route('product');
	...
	public function edit($product_id)
	{
		...
		return view('admin.product.form', compact('product', 'categories', 'images','attribute_sets','role_admins','product_videos', 'product_is_active','languages','brands','affiliate_product','tags'));
	}
	...
	public function verifyName(Request $request)
	{
		...
		$data_name =  $this->product_repository->getByName($product_name,$language_id);
		return response()->json(['name' => $data_name]);
	}
}
