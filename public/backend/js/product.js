...
function get_url_product(){
    ...
    var admin_id = $('#product_manager').val();
    var status_id = $('#product_status').val();
    var before_at = $('#date_1').val();
    var after_at = $('#date_2').val();
      
    var param = '';
    if(brand_id != null)    
        param += '&brand='+brand_id.join(',');
    if(admin_id != null)    
        param += '&admin_id='+admin_id.join(',');
    if(status_id != null)    
        param += '&status_id='+status_id.join(',');
    if(before_at != '')    
        param += '&before_at='+get_english_date(before_at);
    if(after_at != '')    
        param += '&after_at='+get_english_date(after_at);

    return base_url+"admin/product/get-data";
}
...
jQuery(document).ready(function () {
    ...
    if (jQuery('#product_list').length > 0) {
            table = jQuery('#product_list').on('preXhr.dt', function ( e, settings, data ) {
                    var brand_id = $('#brand').val();
                    ...
                    if(before_at != '') 
                        data.before_at = before_at;
                    if(after_at != '')
                        data.after_at = after_at;
                } )
                .dataTable({
                "processing": true,
                "serverSide": true,
                "ajax": {   
                    "url": base_url+"admin/product/get-data",
                    "type": "POST"
                },
                "responsive"   : true,
                "bPaginate"    : true,
                ...
                initComplete : function() {
                    $.ajax({
                        url: base_url + 'admin/product/get-filter',
                        type: 'GET',
                        dataType: 'json',
                    })
                    .done(function(data) {
                        ...
                        $('#product_list_length').append('<div class="btn btn-samall">'+
    ...
    $document.on('click', '#show-resutls', function(event) {
        event.preventDefault();
        table.ajax.url(base_url+"admin/product/get-data").order( [ 0, 'asc' ] ).load();
    });

    $document.on('click', '#show-doublon', function(event) {
        event.preventDefault();
        table.ajax.url(base_url+"admin/product/get-data?show_doublon=show").order( [ 0, 'asc' ] ).load();
    });
...
*/
    ...
    $document.on('keyup keypress change', "#en_product_name", function (e) {
        ...
        var loadurl = base_url+"admin/product/verify-name";
        var language_id = 1;
        var product_name = $("#en_product_name").val();
        if(product_name != ''){
            $.ajax({
                type: "POST",
                url: loadurl,
                data: "product_name=" + product_name + "&language_id=" + language_id,
                
            }).done(function(data) {
                if(data.name != null){
                    $("#en_product_name").css('border-color', 'red');
                    $('.exist-name-english').html('<font color="red">Attention risque de doublon! Vérifiez avant de continuer</font>');
                    $('#add-product').attr('disabled','disabled');
                }else{
                    $("#en_product_name").css('border-color', '#d2d6de');
                    $('.exist-name-english').html('');
    ...
    $document.on('keyup keypress change', "#fr_product_name", function (e) {
        var loadurl = base_url+"admin/product/verify-name";
        var language_id = 2;
        var product_name = $("#fr_product_name").val();
        if(product_name != ''){
            $.ajax({
                type: "POST",
                url: loadurl,
                ...
                if(data.name != null){
                    $("#fr_product_name").css('border-color', 'red');
                    $('.exist-name-french').html('<font color="red">Attention risque de doublon! Vérifiez avant de continuer</font>');
                    $('#add-product').attr('disabled','disabled');
                }else{
                    $("#fr_product_name").css('border-color', '#d2d6de');
                    $('.exist-name-french').html('');
                    $('#add-product').removeAttr('disabled');
                }
            });
        }else{
    ...
    // var inc_change = 0;    
    // var last_values_selected = [];
    //select all
    // $('.select2-multiple').on("change", function(e) {
    //     var val = $(this).val();
    //     if($.inArray('all_', val)!==-1){
    //         if(inc_change == 0){
    //             inc_change++;
    //             var selected = [];
    //             $(this).find("option").each(function(i,e){
    //             if($(e).attr("value")==='all_')
    //                     return true;
    //                     selected[selected.length]=$(e).attr("value");
    //             });
    //             $(this).find('option[value="all_"]').val('clear_');
                
    //         }else{ inc_change = 0; }
    //     }
    // });

    $('.select2-multiple').on("select2:unselecting", function(e){
         if(e.params.args.data.id == 'all_'){
    ...
    $('.date-filter .date input').datepicker({
        format: "dd/mm/yyyy",
        autoclose: "true"                                                   
    });

    $('#date_1').change(function(){
        var date_debut = $('#date_1').val();
        $('#date_2').datepicker('update', date_debut);  
    }); 

    $('#date_2').change(function(){
        var date_debut = $('#date_1').val().split("/");
        var date_fin = $('#date_2').val().split("/");
