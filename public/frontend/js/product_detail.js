$(document).ready(function () {
    ...
    $('#add-to-cart').click(function(event){
        ...
        $('.form-control-product.chosen-select').each(function(index, element){
            if($(element).val() == null){
        ...
        if($('.contain-product-info').find('.invalid').length == 0) {
            $('#product_form').submit();
        }
    });
    $('.form-control-product.chosen-select').change(function(){
         if($(this).hasClass('invalid')){
